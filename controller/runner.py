...
class _TestResult(unittest.TestResult):
    def __init__(self, verbosity=1):
        super(_TestResult, self).__init__(verbosity) #Python 3 可以使用直接使用 super().xxx 代替 super(Class, self).xxx 
...
class _TestRunner:
    def __init__(self, report_dir, report_title, verbosity=1, description=""):
        ...
        self.stop_time = None
        print 'init params:', self
    
    def run(self, test):
        msg = "开始测试，用例数量总共{}个，跳过{}个，实际运行{}个"
        
        log.info(msg.format(Tool.total_case_num, Tool.total_case_num - Tool.actual_case_num,
